...
describe('组件集成测试 (简化版)', () => {
    ...
    test('应该能够协同工作完成完整游戏流程', async () => {
        ...
        for (const [index, question] of questions.entries()) {
            ...
            await scoreManager.recordAnswer(shouldAnswerCorrectly);
        ...
        // 3 correct (+10 each) + 2 wrong (-10 each) = 10
        const wrongAnswers = questionCount - correctAnswers;
        expect(finalStats.score).toBe(correctAnswers * 10 - wrongAnswers * 10);
    ...
    test('应该正确处理数据传递和状态同步', async () => {
        ...
        for (const [index, question] of questions.entries()) {
            ...
            // 模拟答题
            await scoreManager.recordAnswer(true);
            ...
            // 连续答对：第1次+10(1.0),第2次+11(1.1),第3次+12(1.2)
            const expectedScores = [10, 21, 33];
            expect(stats.score).toBe(expectedScores[index]);
        }
    });
    ...
    test('应该正确处理边界情况和错误恢复', async () => {
        ...
        // 测试大量题目生成 (2-9范围，最多8x8=64种组合)
        const maxQuestions = questionGenerator.generateQuestions(64);
        expect(maxQuestions).toHaveLength(64);

        // 验证所有题目都在2-9范围内
        maxQuestions.forEach(question => {
            expect(question.multiplicand).toBeGreaterThanOrEqual(2);
            expect(question.multiplicand).toBeLessThanOrEqual(9);
            expect(question.multiplier).toBeGreaterThanOrEqual(2);
        ...
        await scoreManager.recordAnswer(true);
        await scoreManager.recordAnswer(false);
    ...
    test('应该维护组件状态的一致性', async () => {
        ...
        for (let round = 1; round <= 3; round++) {
            ...
            for (const question of questions) {
                await scoreManager.recordAnswer(Math.random() > 0.5); // 随机答题
            }
